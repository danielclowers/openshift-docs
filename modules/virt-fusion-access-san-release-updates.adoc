...
Issue with Fusion Access for SAN filesystem provisioning::
Fusion Access for SAN does not pause deletion of core pods while a filesystem and its LocalDisks are being provisioned. If this happens, a filesystem might be only partially created on the LUN, which causes the following persistent error:
+
[source,terminal]
----
...
----
+
There is no workaround for this issue. Contact IBM Support for assistance.
+
