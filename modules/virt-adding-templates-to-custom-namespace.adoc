...
* You created a custom namespace for templates.
...
. Add the `commonTemplatesNamespace` field and set your target namespace. Example:
+
[source,yaml]
----
...
spec:
  commonTemplatesNamespace: <custom_namespace> <1>
----
<1> The custom namespace where the operator creates the templates.
